...
export class Group extends Notifier {
  public get storage(): GroupStorage {
  ...
  public static fromJson(storage: GroupStorage, json: any): Group {
    const title = json[keyTitle] as string;
    const group = new Group(storage, title, []);
    group.replaceAllBookmarks(
      (json[keyBookmarks] as []).map((j: any) => Bookmark.fromJson(group, j)),
    );
    return group;
  }
  ...
  public addBookmark(url: URL, title: string, icon?: Buffer): Bookmark {
  ...
  public replaceAllBookmarks(b: Bookmark[]): void {
