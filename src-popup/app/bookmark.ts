...
export class Bookmark extends Notifier {
  ...
  public get icon(): Buffer | undefined {
    return this._icon;
  }
  ...
  public static fromJson(group: Group, json: any): Bookmark {
    const url = new URL(json[keyUrl]);
    const title = json[keyTitle] as string;
    const icon =
      json[keyIcon] != null ? Buffer.from(json[keyIcon], 'base64') : undefined;
    return new Bookmark(group, url, title, icon);
  }

  public clone(parentGroup: Group): Bookmark {
    return new Bookmark(parentGroup, this._url, this._title, this._icon);
  }

  private static bufferEqual(b1: Buffer | undefined, b2: Buffer | undefined) {
