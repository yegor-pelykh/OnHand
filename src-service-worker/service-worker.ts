...
abstract class ConnectionManager {
  private static connectedPorts: Set<chrome.runtime.Port> =
    new Set<chrome.runtime.Port>();
  private static awaitingMessages: Map<string, MessageConfirmation> = new Map<
    string,
    MessageConfirmation
  >();
  ...
  static async broadcastMessage(
    ...
    const promises = Array.from(this.connectedPorts.values()).map((p) =>
      this.sendMessage(p, type, data, error),
    );
    return await Promise.allSettled(promises);
  }

  static async handlerConvertToPng(
    port: chrome.runtime.Port,
    data: ToPngMessageData,
    error: any,
  ) {
...
abstract class ActionHandler {
  private static allowedSchemes: string[] = ['https', 'http'];
